...
const PythonButton = ({openPython, onClose, children}) => {
    const {DarkMode} = useContext(ThemeContext);
  return (
    <div onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-colors 
    ${openPython ? "visible bg-black/20" : "invisible"}`}>
        
    <div onClick={(e) => e.stopPropagation()} className={`rounded-xl shadow p-6 transition-all ${openPython ? "scale-100 opacity-100" : "scale-125 opacity-0"} ${DarkMode ? "bg-slate-800" : "bg-neutral-100"}`}>
